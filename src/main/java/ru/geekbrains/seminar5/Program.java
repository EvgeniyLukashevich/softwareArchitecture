                ...
                try {
                    switch (no) {
                        ...
                        case 7:
                            ...
                            // Реализовал один из вариантов добавления модели в проект
                        case 8:
                            System.out.print("Укажите необходимое количество текстур для модели: ");
                            if (scanner.hasNextInt()) {
                                int texturesQuantity = scanner.nextInt();
                                scanner.nextLine();
                                System.out.println("\n ### ### ### ### ###\n");
                                editor3D.addModel(texturesQuantity);
                                System.out.println("\n ### ### ### ### ###\n");
                            } else {
                                System.out.println("Количество текстур указано некорректно.");
                            }
                            break;
                        default:
...
class Editor3D implements UILayer {
    ...
    public void addModel(int texturesQuantity) {
        // Предусловие
        checkProjectFile();
        System.out.println("Начат процесс создания модели.");
        businessLogicalLayer.addModel(texturesQuantity);
...
interface UILayer {
    ...
    void addModel(int texturesQuantity);
...
class EditorDatabaseAccess implements DatabaseAccess {
    ...
    public void addModel(Model3D model) {
        if (editorDatabase != null)
            System.out.println("Связь с БД установлена");
        editorDatabase.addModel(model);
    }
...
class EditorDatabase implements Database {
    ...
    public void addModel(Model3D model) {
        ...
        entities.add(model);
        System.out.println("Модель добавлена в проект и готова к сохранению.");
        this.save();
